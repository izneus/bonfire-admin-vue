  ...
  <div class="app-container" :class="{'has-bulk':selectedAuthority.length > 0}">
        ...
        <el-row :gutter="24">
          <el-col :span="6">
            <el-form-item label="权限名称:" prop="authority">
              <el-input v-model="query.authority" placeholder="输入权限名称" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
    ...
    <div class="table-wrapper">
      ...
      >
        ...
        <el-table-column prop="authority" label="权限名称" show-overflow-tooltip />
          ...
          <el-row :gutter="30">
            ...
            <el-col :span="24">
              <el-form-item label="备注" prop="remark">
                <el-input v-model="authority.remark" type="textarea" :rows="5" placeholder="输入备注" />
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
...
export default {
  ...
  methods: {
    ...
    handleQuery() {
      ...
      listAuthorities(this.query).then(res => {
    ...
    editAuthority(authorityId) {
      ...
      getAuthority({ id: authorityId }).then(res => {
    ...
    handleCreateAuthority() {
      this.$refs.authorityForm.validate(valid => {
        if (valid) {
          ...
          // 请求api
          createAuthority(this.authority).then(res => {
            // 成功请求弹出提示
            this.$message({
              showClose: true,
              message: '创建成功',
    ...
    handleUpdateAuthority() {
      this.$refs.editAuthorityForm.validate(valid => {
        if (valid) {
          this.createLoading = true
          updateAuthority(this.authority).then(res => {
    ...
    handleDeleteAuthorities() {
      ...
      deleteAuthorities({ ids }).then(res => {
        // 成功请求弹出提示
        this.$message({
          showClose: true,
          message: '批量删除成功',
