...
export default {
  ...
  methods: {
    ...
    getActiveElement() {
      ...
      this.initFormOnChanged(null, "null");
      this.bpmnModeler.on("import.done", e => {
        this.initFormOnChanged(null, 'import.done');
      });
      // 监听选择事件，修改当前激活的元素以及表单
      this.bpmnModeler.on("selection.changed", ({ newSelection }) => {
        this.initFormOnChanged(newSelection[0] || null, 'selection.changed');
      });
      this.bpmnModeler.on("element.changed", ({ element }) => {
        // 保证 修改 "默认流转路径" 类似需要修改多个元素的事件发生的时候，更新表单的元素与原选中元素不一致。
        if (element && element.id === this.elementId) {
          this.initFormOnChanged(element, 'element.changed');
        }
      });
    },
    // 初始化数据
    initFormOnChanged(element, ss) {
      Log.prettySuccess('事件类型：', ss)
